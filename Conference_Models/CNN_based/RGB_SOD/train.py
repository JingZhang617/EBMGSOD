...
parser.add_argument('--batchsize', type=int, default=29, help='training batch size')
...
image_root = './RGB_Dataset/train/DUTS/img/'
gt_root = './RGB_Dataset/train/DUTS/gt/'
...
for epoch in range(1, (opt.epoch+1)):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            pred_init, pred_ref = generator(images,z_g_noise)
            loss_all = structure_loss(pred_init, gts) + structure_loss(pred_ref, gts)
            ...
            visualize_init(torch.sigmoid(pred_init))
            visualize_ref(torch.sigmoid(pred_ref))
            visualize_gt(gts)
