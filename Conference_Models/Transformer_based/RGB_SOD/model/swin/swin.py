...
class Swin(torch.nn.Module):
    def __init__(self, img_size,latent_dim):
        ...
        self.layer5 = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], 1, channel_size*2)
        self.noise_conv = nn.Conv2d(channel_size + latent_dim, channel_size, kernel_size=1, padding=0)
    ...
    def forward(self, x, z, depth=None):
        # print(z.size())
        if depth != None:
        ...
        z_noise = z
        z_noise = self.tile(z_noise, 2, x4.shape[self.spatial_axes[0]])
        z_noise = self.tile(z_noise, 3, x4.shape[self.spatial_axes[1]])

        x5 = torch.cat((x5, z_noise), 1)
        x5 = self.noise_conv(x5)

        pred = self.sal_dec(x1,x2,x3,x4,x5)
...
class Saliency_feat_decoder(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv54 = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], channel,
                                            2 * channel)

        self.conv543 = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], channel,
                                            3 * channel)
        self.conv5432 = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], channel,
                                             4 * channel)
        self.conv54321 = self._make_pred_layer(Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], channel,
                                              5 * channel)
