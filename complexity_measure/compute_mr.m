...
% Objectnes Measuew. In ICIP, 2015. (SOM)

% [2] Wangjiang Zhu, Shuang Liang, Yichen Wei, and Jian Sun. Saliency
% Optimization from Robust Background Detection. In CVPR, 2014. (RBD)
...
% background priors. In ECCV, 2012. (GS)

% [5] C. Yang, L. Zhang, H. Lu, X. Ruan, and M.-H. Yang. Saliency
% detection via graph-based manifold ranking. In CVPR, 2013. (MR)
...
dataset = 'MSRABTest';
SRC = ['/home/jing-zhang/jing_file/RGB_sal_dataset/test/img/' dataset];       %Path of input images
...
% RES1 = '/home/jing-zhang/jing_file/competing_methods/handcrafted/Saliency-Objectness-master/SOM';
RES5 = ['/home/jing-zhang/jing_file/competing_methods/handcrafted/Saliency-Objectness-master/test/MR/' dataset];
...
%  
%     smapName=fullfile(RES1, strcat(noSuffixName, '_SOM.png'));
...
%     optwCtr = SaliencyOptimization(adjcMatrix, bdIds, colDistM, neiSigma, bgWeight, wCtr);
%     
%     smapName=fullfile(RES2, strcat(noSuffixName, '_RBD.png'));
%     SaveSaliencyMap(optwCtr, pixelList, frameRecord, smapName, true);
%     
%     %% Saliency Filter
%     [cmbVal, contrast, distribution] = SaliencyFilter(colDistM, posDistM, meanPos);
%     
%     smapName=fullfile(RES3, strcat(noSuffixName, '_SF.png'));
...
%     
%     smapName=fullfile(RES4, strcat(noSuffixName, '_GS.png'));
%     SaveSaliencyMap(geoDist, pixelList, frameRecord, smapName, true);
    ...
    smapName=fullfile(RES5, strcat(noSuffixName, '.png'));
