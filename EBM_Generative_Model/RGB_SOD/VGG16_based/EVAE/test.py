...
model.load_state_dict(torch.load('./models/VGG/Model_30_gen.pth'))
model.cuda()
model.eval()

ebm_model = EBM_Prior(opt.ebm_out_dim, opt.ebm_middle_dim, opt.latent_dim)
ebm_model.load_state_dict(torch.load('./models/VGG/Model_30_ebm.pth'))
ebm_model.cuda()
ebm_model.eval()
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(opt.iter_num):
            ...
            z_e_0 = model(image, z_prior0, z_post0, image, prior_z_flag=True, istraining=False)
            ...
            generator_pred = model.forward(image, z_e_noise, z_post0, image, prior_z_flag=False, istraining=False)
            res = generator_pred
