...
ebm_model.load_state_dict(torch.load('./models/Model_50_ebm.pth'))
ebm_model.cuda()
ebm_model.eval()
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(opt.iter_num):
            ...
            generator_pred = generator.forward(image, z_e_noise, z_post0, image, prior_z_flag=False, istraining = False)
            res = generator_pred
