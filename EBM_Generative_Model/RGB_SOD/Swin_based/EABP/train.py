...
def train_one_epoch(model, generator_optimizer, ebm_model, ebm_optimizer, train_loader, loss_fun):
    ...
    print('Generator Learning Rate: {:.2e}'.format(generator_optimizer.param_groups[0]['lr']))
    ...
    for i, pack in enumerate(progress_bar):
        for rate in size_rates:
            ...
            if task_name == 'RGBD-SOD':
                ...
                visualize_all(torch.sigmoid(ref_pre), gts, option['log_path'])

                if rate == 1:
                    loss_record.update(loss.data, option['batch_size'])
                    loss_record_ebm.update(loss.data, option['batch_size'])
...
if __name__ == "__main__":
    ...
    ebm_optimizer, scheduler_ebm = get_optim_ebm(option, ebm_model.parameters())
    task_name = option['task']
    if task_name == 'RGBD-SOD':
    ...
    for epoch in range(1, (option['epoch']+1)):
        model, ebm_model, loss_record, loss_record_ebm = train_one_epoch(model, optimizer, ebm_model, ebm_optimizer, train_loader, loss_fun)
        writer.add_scalar('gen loss', loss_record.show(), epoch)
        writer.add_scalar('ebm loss', loss_record_ebm.show(), epoch)
        writer.add_scalar('lr', optimizer.param_groups[0]['lr'], epoch)
        writer.add_scalar('lr', ebm_optimizer.param_groups[0]['lr'], epoch)
        ...
        if epoch % option['save_epoch'] == 0:
            ...
            torch.save(ebm_model.state_dict(), save_path + '/{:d}_{:.4f}'.format(epoch, loss_record_ebm.show()) + '_ebm.pth')
