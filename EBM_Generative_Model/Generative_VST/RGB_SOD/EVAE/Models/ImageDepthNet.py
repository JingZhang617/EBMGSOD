...
class ImageDepthNet(nn.Module):
    def __init__(self, args):
        ...
        self.enc_x = Encode_x(3, args.latent_dim)
        self.enc_xy = Encode_xy(4, args.latent_dim)
    ...
    def forward(self, image_Input, z_prior0, z_post0, y=None, prior_z_flag=True, istraining = True):

        B, _, _, _ = image_Input.shape
        # VST Encoder
        rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4 = self.rgb_backbone(image_Input)
        mu_prior, logvar_prior, dist_prior = self.enc_x(image_Input)
        ...
        if istraining == False:
            if prior_z_flag == True:
                ...
                pred = self.prior_dec(rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4, z_prior0)
                return pred
        else:
            mu_post, logvar_post, dist_post = self.enc_xy(torch.cat((image_Input, y), 1))
            kld = torch.mean(self.kl_divergence(dist_post, dist_prior))
            if prior_z_flag == True:
                z_post = self.reparametrize(mu_post, logvar_post)
                return z_prior, z_post
            else:
                pred_prior = self.prior_dec(rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4, z_prior0)
                pred_post = self.post_dec(rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4, z_post0)
...
class Decoder_SOD(nn.Module):
    ...
    def forward(self, rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4, z_noise):
