...
from model.ResNet_models import Pred_endecoder, EBM_Prior, FCDiscriminator
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            for kk in range(opt.g_l_steps):
                gen_res = generator(images, depths, z)
            ...
            pred_init = generator(images, depths, z_g_noise)
            loss_all = structure_loss(pred_init, gts)
            Dis_output = discrminator(images, torch.sigmoid(pred_init).detach())
