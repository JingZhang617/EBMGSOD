...
discriminator.load_state_dict(torch.load('./models/Model_100_ebm.pth'))
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(opt.iter_num):
            ...
            z_e_0 = generator(image, depth, z_prior0, z_post0, image, prior_z_flag=True, istraining=False)
            ...
            generator_pred = generator.forward(image, depth,z_e_noise, z_post0, image, prior_z_flag=False, istraining=False)
            res = generator_pred
    ...
    print('[INFO] Avg. Time used in this sequence: {:.4f}s'.format(np.mean(time_list)))
            ...
            # cv2.imwrite(save_path_base+name, res)
