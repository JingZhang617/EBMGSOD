...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.depth_cov = nn.Conv2d(
            6, 3, kernel_size=3, stride=1, padding=1, bias=True
        )
        self.enc_x = Encode_x(6, channel, latent_dim)
        self.enc_xy = Encode_xy(7, channel, latent_dim)
    ...
    def forward(self, x, depth, z_prior0, z_post0, y=None, prior_z_flag=True, istraining = True):
        raw_x = x
        ...
        mu_prior, logvar_prior, dist_prior = self.enc_x(torch.cat((raw_x,depth),1))
        ...
        if istraining == False:
            ...
            mu_post, logvar_post, dist_post = self.enc_xy(torch.cat((raw_x, depth, y), 1))
            kld = torch.mean(self.kl_divergence(dist_post, dist_prior))
            if prior_z_flag == True:
                z_post = self.reparametrize(mu_post, logvar_post)
                return z_prior, z_post
            else:
                pred_prior = self.prior_dec(x1, x2, x3, x4, z_prior0)
                pred_post = self.post_dec(x1, x2, x3, x4, z_post0)
...
class Pred_decoder(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, z):
        ...
        z_noise = self.tile(z, 2, conv4_feat.shape[self.spatial_axes[0]])
