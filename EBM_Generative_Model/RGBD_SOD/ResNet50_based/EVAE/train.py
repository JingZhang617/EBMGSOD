...
scheduler_gen = lr_scheduler.StepLR(generator_optimizer, step_size=opt.decay_epoch, gamma=opt.decay_rate)
...
for epoch in range(1, (opt.epoch+1)):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            z_e_0, z_g_0 = generator(images, depths, z_prior0, z_post0, gts, prior_z_flag=True, istraining=True)
            ...
            for kk in range(opt.g_l_steps):
                _, _, _, gen_res, _ = generator(images, depths, z_prior0, z, gts, prior_z_flag=False, istraining=True)
            ...
            _, _, pred_prior, pred_post, latent_loss = generator(images, depths, z_e_noise, z_g_noise, gts, prior_z_flag=False,
                                                                 istraining=True)
            ...
            # # ## train discriminator
            # dis_pred = torch.sigmoid(pred_init).detach()
        ...
        if i % 10 == 0 or i == total_step:
            print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Gen Loss: {:.4f}, EBM Loss: {:.4f}'.
