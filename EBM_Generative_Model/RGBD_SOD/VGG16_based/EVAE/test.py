...
parser.add_argument('--iter_num', type=int, default=10, help='training batch size')
...
model.load_state_dict(torch.load('./models/VGG/Model_100_gen.pth'))
model.cuda()
model.eval()

discriminator = EBM_Prior(opt.ebm_out_dim, opt.ebm_middle_dim, opt.latent_dim)
discriminator.load_state_dict(torch.load('./models/VGG/Model_100_ebm.pth'))

discriminator.cuda()
discriminator.eval()

test_datasets = ['NJU2K','NLPR', 'LFSD', 'DES', 'SIP', 'STERE']
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(opt.iter_num):
            z_prior0 = sample_p_0(n=image.shape[0])
            z_post0 = sample_p_0(n=image.shape[0])

            z_e_0 = model(image, depth, z_prior0, z_post0, image, prior_z_flag=True, istraining=False)
            ...
            generator_pred = model.forward(image, depth,z_e_noise, z_post0, image, prior_z_flag=False, istraining=False)
            res = generator_pred
