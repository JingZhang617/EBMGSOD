...
def train_one_epoch(model, generator_optimizer, discriminator, discriminator_optimizer, ebm_model, ebm_optimizer, train_loader, loss_fun):
    model.train()
    discriminator.train()
    ebm_model.train()
    ...
    for i, pack in enumerate(progress_bar):
        for rate in size_rates:
            ...
            if task_name == 'RGBD-SOD':
                ...
                # z_noise = torch.randn(images.shape[0], option['latent_dim']).cuda()

                ## learn generator
                pred = model(images, z_g_noise, depths)
                loss_gen = cal_loss(pred, gts, loss_fun)
                Dis_output = discriminator(images, torch.sigmoid(pred).detach())
                ...
                loss_all = loss_gen + 0.1 * loss_dis_output
                ...
                visualize_all(torch.sigmoid(pred), gts, option['log_path'])
        ...
        progress_bar.set_postfix(gen_loss=f'{loss_record.show():.5f}',dis_loss=f'{loss_record_dis.show():.5f}',ebm_loss=f'{loss_record_ebm.show():.5f}')
    ...
    return model,discriminator,ebm_model, loss_record, loss_record_dis, loss_record_ebm
...
if __name__ == "__main__":
    ...
    model, discriminator, ebm_model = get_model(option)
    ...
    for epoch in range(1, (option['epoch']+1)):
        model, discriminator, ebm_model, loss_record, loss_record_dis, loss_record_ebm = train_one_epoch(model, optimizer, discriminator, optimizer_dis, ebm_model, optimizer_ebm, train_loader, loss_fun)
