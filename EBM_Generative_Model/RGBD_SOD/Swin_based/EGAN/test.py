...
generator.load_state_dict(torch.load(option['ckpt_save_path']+'/100_0.1978_gen.pth'))
generator.cuda()
generator.eval()

ebm_model.load_state_dict(torch.load(option['ckpt_save_path']+'/100_-0.0170_ebm.pth'))
ebm_model.cuda()
ebm_model.eval()
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(option['iter_num']):
            ...
            res = generator.forward(image, z_e_noise, depth)  # Inference and get the last one of the output list
