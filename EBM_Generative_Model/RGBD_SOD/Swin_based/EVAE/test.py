...
generator.load_state_dict(torch.load(option['ckpt_save_path']+'/100_0.2536_gen.pth'))
generator.cuda()
generator.eval()

ebm_model.load_state_dict(torch.load(option['ckpt_save_path']+'/100_0.1171_ebm.pth'))
ebm_model.cuda()
ebm_model.eval()
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        for iter in range(option['iter_num']):
            ...
            z_e_0 = generator(image, z_prior0, z_post0, depth, image, prior_z_flag=True, istraining=False)
            ...
            res = generator.forward(image, z_e_noise, z_post0, depth, image, prior_z_flag=False, istraining = False)
