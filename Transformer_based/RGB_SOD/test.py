...
generator.load_state_dict(torch.load(option['ckpt_save_path']+'/30_0.1147_gen.pth'))
generator.eval()
ebm_model.load_state_dict(torch.load(option['ckpt_save_path']+'/30_-0.0051_ebm.pth'))
ebm_model.eval()
...
for dataset in test_datasets:
    ...
    for i in tqdm(range(test_loader.size), desc=dataset):
        ...
        res = generator.forward(image,z_e_noise)[-1]   # Inference and get the last one of the output list
