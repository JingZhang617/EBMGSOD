...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.sod_dec = Saliency_feat_decoder(channel)
        self.sod_dec_ha = Saliency_feat_decoder(channel)
        ...
        self.depth_cov = nn.Conv2d(
            4, 3, kernel_size=3, stride=1, padding=1, bias=True
        )
    ...
    def forward(self, x, depth, z):
        ...
        pred = self.sod_dec(conv1_feat,conv2_feat,conv3_feat,conv4_feat)

        x2_2 = self.ha(self.upsample0125(pred).sigmoid(), x2)
        x3_2 = self.resnet.layer3_2(x2_2)  # 1024 x 16 x 16
        x4_2 = self.resnet.layer4_2(x3_2)  # 2048 x 8 x 8

        pred_ref = self.sod_dec_ha(conv1_feat, self.conv2_2(x2_2), self.conv3_2(x3_2), self.conv4_2(x4_2))

        return pred, pred_ref
...
class Saliency_feat_decoder(nn.Module):
    ...
    def forward(self, conv1_feat,conv2_feat,conv3_feat,conv4_feat):
        ...
        sal_init = self.upsample4(self.clc_layer(conv4321))
